...
QZFL.css.insertCSSLink("/css/style.css", {
	...
	insertCSSLink: function(url, opts){
		...
		try{
			url && (cssLink.href = (url).replace(/^http:\/\//,window.location.protocol+'//'));
		}catch(ign){}
...
QZFL.event = {
	...
	delegate : function(delegateDom, selector, eventType, fn, argsArray) {
	    ...
	    //pre condition
	    if(!delegateDom || !selector || !eventType || !fn) {
	        return res;
	    }
	    //init data
	    if (!delegateDom.eventsListUID) {
	        delegateDom.eventsListUID = "e" + (++E._objSeqUID);
	    }
	    if (!(eventData = E._eventListDictionary[delegateDom.eventsListUID])) {
	        eventData = E._eventListDictionary[delegateDom.eventsListUID] = {};
	    }
	    if (!fn.__elUID) {
	        fn.__elUID = "e" + (++E._fnSeqUID) + delegateDom.eventsListUID;
	    }

	    //初始化事件类型
	    delegateType = (E.delegate._specialEvent[eventType] || {}).delegateType || eventType;
	    
	    if (!eventData[delegateType]) {
	        eventData[delegateType] = {};
	    }
	    
	    handler = eventData.delegateHandler;
	    if(!handler) {
	        //单事件注册
	        handler = eventData.delegateHandler = function(e) {
	            E.delegate._dispatch.apply(delegateDom, arguments);
	        };
	    }
	    
	    //初始化事件 - 对于一个代理对象，一种事件类型，仅注册一次事件
	    handlerArray = eventData[delegateType].delegateHandlers;
	    if(!handlerArray) {
	        handlerArray = eventData[delegateType].delegateHandlers = [];
	        eventData.delegateCount = (eventData.delegateCount || 0) + 1;
	        //注册事件
	        if(delegateDom.addEventListener) {
	            delegateDom.addEventListener(delegateType, handler, false);
	            res = true;
	        }else if(delegateDom.attachEvent) {
	            delegateDom.attachEvent('on' + delegateType, handler);
	            res = true;
	        }else{
	            return res;
	        }
	        ...
	        for(var i = 0, len = handlerArray.length; i < len; i++){
	            if(handlerArray[i].guid === fn.__elUID){
	                return false;
	            }
	        }
	        res = true;
	    ...
	    handlerObj = {
	        ...
	        'argsArray' : argsArray,
	        'origType' : eventType,
	        'quick' : E.delegate._quickParse(selector)
	    }
	    //store
	    handlerArray.push(handlerObj);
	    return res;
	},
	/**
     * @description 取消事件代理
	...
	undelegate : function(delegateDom, selector, eventType, fn) {
	    ...
	    if(!eventType) {
	        for(var j in eventData) {
	            arguments.callee(delegateDom, selector, j, fn);
	        }
	        return;
	    }
	    
	    eventType = (E.delegate._specialEvent[eventType] || {}).delegateType || eventType;
	    
	    handlerArray = (eventData[eventType] || {}).delegateHandlers;
	    if(!handlerArray){
	        return;
	    }
	    
	    len = handlerArray.length;
	    if(fn || selector) {
	        for(var i = 0, h; i < handlerArray.length;) {
	            h = handlerArray[i];
	            if((typeof(fn) == 'function' && h.guid === fn.__elUID) || (selector === h.selector)) {
	        ...
	        handlerArray.length = 0;  //清除数据
	    }

	    //clear
	    if(handlerArray.length === 0 && len !== 0) {
	        delegateType = (E.delegate._specialEvent[eventType] || {}).delegateType || eventType;
	        if(delegateDom.removeEventListener) {
	            delegateDom.removeEventListener(delegateType, eventData.delegateHandler, false);
	        } else if(delegateDom.detachEvent) {
	            delegateDom.detachEvent('on' + delegateType, eventData.delegateHandler);
	        }
	        
	        handlerArray = null;
	        delete eventData[eventType].delegateHandlers;

	        eventData.delegateCount--;
	        if(eventData.delegateCount === 0){
	            delete eventData.delegateHandler;
	            delete eventData.delegateCount;
	        }
	    }   
	}
};
...
QZFL.event.delegate._quickParse = function(selector) {
    var REG = /^(\w*)(?:#([\w\-]+))?(?:\.([\w\-]+))?$/, //[all, tag, id, class]
        quick = REG.exec(selector);
    if(quick) {
...
QZFL.event.delegate._isQuickMatch = function(elem, m) {
    return (
...
QZFL.object.each({
    ...
    'mouseleave' : 'mouseout'
}, function(delegateType, origType) {
    QZFL.event.delegate._specialEvent[origType] = {
        'delegateType' : delegateType,
        'handler' : function(event, handlerObj) {
            var target = this,
                relatedTarget = QZFL.event.getRelatedTarget(event);

            if(target !== relatedTarget && !QZFL.dom.isAncestor(target, relatedTarget)) {
                handlerObj.handler.apply(this, [event].concat(handlerObj.argsArray));
            }
        }
    }
});

QZFL.event.delegate._dispatch = function(event) {
    var E = QZFL.event,
    ...
    eventData = E._eventListDictionary[this.eventsListUID] || {};
    handlerArray = (eventData[type] || {}).delegateHandlers || [];
    
    for(curDom = E.getTarget(event); curDom != this; curDom = curDom.parentNode || this) {
        for(var i = 0, len = handlerArray.length, handlerObj; i < len; i++) {
            handlerObj = handlerArray[i];
            selector = handlerObj.selector;
            hit = (handlerObj.quick ? E.delegate._isQuickMatch(curDom, handlerObj.quick) : (Sizzle && Sizzle.matchesSelector(curDom, selector)));

            if(hit) {
                specialHandler = (E.delegate._specialEvent[handlerObj.origType] || {}).handler;
                //fire
                if(specialHandler){
                    specialHandler.apply(curDom, [event, handlerObj]);
                }else{
                    handlerObj.handler.apply(curDom, [event].concat(handlerObj.argsArray));
                }
            }
        }
    }
...
QZFL.effect = {
	...
	run : function(elem, prop, opts){
		...
		if(o.off){
			...
			if(o.mode[1]=='css3' && opts.change == QZFL.emptyFn){
				//纯css3动画
...
var _autoXSS = (function () {
	...
	var _XSS = {
		go:function (item) {
			...
			if(itemType==='string') {
				...
				if(itemObjType === '[object Array]' || itemObjType === '[object Arguments]') {
		...
		filter:function (str) {
			str = str || '';
			str = this.decodeHtml(str);//有时后台返回的数据已经encode了，若直接再次encode会出问题，故先decode一下
	...
	return function (data) {
		...
		//因处理xss对性能影响较大，暂时只处理认证空间的，pathname中有/qzone/biz/表示认证空间
			return data;
		} else {
			return _XSS.go(data);
		}
...
function isSameDomain(url) {
	var checkUrl = url;
	if(url.indexOf('//')===0){
		checkUrl = window.location.protocol + checkUrl;
	}

	if (checkUrl.charAt(0) == '/') {
		return true;
	}
	var rs = /^https?\:\/\/([\s\S]+?)\//.test(checkUrl) && RegExp.$1 == cdm;
	return rs;
}
...
function reportSSLInfo(sslinfo,options){
	if(!sslinfo){
		return;
	}
	...
	//$stgw_request_id|$request_time|$upstream_response_time|$connection_requests|$ssl_session_reused|$ssl_ocsp_status|$ssl_protocol|$ssl_cipher|$tcpinfo_rtt|0;
	var frags  = (sslinfo).split('|');
	var isSessionReused = frags[4] == 'r';
	var connectionNum = parseInt(frags[3] || '0',10);
	if(connectionNum==0 || isNaN(connectionNum)){
		return;//数据异常，不要上报了
	}
	if(window.haboStat){
		//connectionNum为1表示tcp重新链接，只有tcp重连的时候才有可能复用session
		if(connectionNum==1){
			window.haboStat({
		...
		window.haboStat({
       		...
       		stime : 1000, 
       		resultcode : connectionNum > 1 ? 0 : 1,
       		touin : window.g_iLoginUin || 0,
       		frequency :  1
    	});	
...
function ajaxRequest (url, data, opt) {
	...
	if (isSameDomain(url)) {
		...
		xhr.onreadystatechange = function() {
			...
			if (this.readyState == 4) {
					//endTime暂时记录着，但是没有上报,opt.instance一般只有域名收归才有
					if (this.status == 200) {
						...
						if(o && o.code == -3005) {
							var that = this;
	                     	window.seajs && seajs.use('//qzonestyle.gtimg.cn/qzone/hybrid/lib/pow/pow', function(pow){
			                    if(!pow){
			                    ...
			                    pow.getResult({
			                        ...
			                        url = url + (url.indexOf('?') > -1 ? '&' : '?') + 'pow=' + o.target + '&powret=' + result;
						...
						reportSSLInfo(this.getResponseHeader('x-stgw-ssl-info'),{
							url : url
						});
	...
	// 非ie以及ie10已全面支持了xmlhttprequest level2
		...
		xhr.onload = function () {
			...
			// //验证码逻辑
			// if(o.subcode == 402) {
			// 	QZFL.imports('https://qzonestyle.gtimg.cn/qzone/v6/interface/fm/_specialPopup.js', function() {
			//         QZONE.FP.showVerifyBox(o.subcode, function(verifyCode) {
		 //                if (verifyCode) {
	...
	setHeader(xhr, opt.method.toLowerCase() == 'post' ? extend({
...
QZFL.JsLoader.prototype._load = (function() {
	...
	return function(/*sId, */src, doc, opts){
		...
		if(QZFL.win.g_cdn_proto == 'https:' || window.location.protocol =='https:'){
			if(src.indexOf('/')===0 && src.indexOf('//')!==0){ //表示当前url是相对路径而不是相对的协议
				src = window.location.protocol + '//' + (window.siDomain || 'qzonestyle.gtimg.cn') + (src.indexOf('/')===0 ? src : '/'+src);
			}
			_js.src = src.replace(/^http:/, QZFL.win.g_cdn_proto || window.location.protocol);
		}else{
			_js.src = src;	
		}
...
QZFL.imports.load = function(url, scb, ecb, opt){
	...
	if(window.QZFL && QZFL.win && (QZFL.win.g_cdn_proto == 'https:' || window.location.protocol =='https:')){
		formatUrl = formatUrl.replace(/^http:/, QZFL.win.g_cdn_proto || window.location.protocol)
	}
	if (QZFL.imports.urlCache[formatUrl] === true) {
		setTimeout(function(){
			if (QZFL.lang.isFunction(scb)) scb()
		}, 0);
		return;
	}
	if (!QZFL.imports.urlCache[formatUrl]) {
		QZFL.imports.urlCache[formatUrl] = [];
		var loader = new QZFL.JsLoader();
		loader.onload = function(){
			QZFL.imports.execFnQueue(QZFL.imports.urlCache[formatUrl], 1);
			QZFL.imports.doOn("load",{'url':formatUrl});
			QZFL.imports.urlCache[formatUrl] = true;
		};
		loader.onerror = function(){
			QZFL.imports.execFnQueue(QZFL.imports.urlCache[formatUrl], 0);
			QZFL.imports.doOn("error",{'url':formatUrl});
			QZFL.imports.urlCache[formatUrl] = null;
			delete QZFL.imports.urlCache[formatUrl];
		};
		loader.load(formatUrl, null, opt);
	}
	QZFL.imports.urlCache[formatUrl].push([ecb, scb]);
};
...
QZFL.FormSender.prototype.send = (function() {
	...
	return function() {
		...
		var isspeedup = cookieflag && (/(?:^user\.(s\d\.)?|\d{5,}\.|rc\.)qzone\.qq\.com/.test(host)),
			...
			ajaxurl = realurl,
			path, tmplURI,
			canUseXHR = win.QZFL.config.canUseXHR2Browser();
		...
		if (this.charset == 'utf-8') {
			if (canUseXHR
				...
				var targetUrl = transUrl2HttpsIfNeed(realurl);
				targetUrl =  targetUrl+ ( targetUrl.indexOf('?')>-1 ? '&' : '?')+ (QZFL.win.g_qzonetoken ? 'qzonetoken='+QZFL.win.g_qzonetoken : '');
				tmplURI = QZFL.util.URI(targetUrl);
				this.speedup = 1;
				var th = this;
				return (this.type = 'xhr'), win.QZFL.ajaxPost( targetUrl, data, {
					onload: function(rsp){
						// debugger;
						try{
							//跨域的话会取不到这个头而报错，所以try一下
							th._sslinfo = this.getResponseHeader('x-stgw-ssl-info') || '';
						}catch(err){}
						th.endTime = +new Date;
						th.onSuccess(rsp);
					},
			...
			sender.callback = (function(th) {
				return function(){
					...
					if (arguments && arguments[0].code !== 0) {
						var _time = 0;
						if(th.endTime != undefined && th.startTime != undefined) {
							_time = th.endTime - th.startTime;
						}
						errcodeReport({
		...
		if(!this.instanceForm){
			var t = this, ie = QZFL.userAgent.ie,ff =QZFL.userAgent.firefox,  ifrurl, ifrHTML,data = t.data;
            
            
            var targetURL = t.uri;
            // 因为跨域问题，formsender不能从http
            targetURL = transUrl2HttpsIfNeed(targetURL, true);
            //先加上g_tk和qzonetoken
            targetURL = targetURL + (targetURL.indexOf('?') > -1 ? '&' : '?') + 'g_tk=' + QZFL.pluginsDefine.getACSRFToken(targetURL) +(QZFL.win.g_qzonetoken ? '&qzonetoken='+QZFL.win.g_qzonetoken : '');
			...
			ifrHTML = ifrHTML + '<script type="text/javascript">' + ((ie || ff) && ('document.charset="' + t.charset + '"')||'') + ';document.domain="' + document.domain + '";frameElement.submited=void(0);frameElement.state="sending";<\/script><\/head><body>';
			ifrHTML = ifrHTML + '<form action="'+ targetURL + '" accept-charset="' + t.charset + '" id="p" enctype="application/x-www-form-urlencoded;charset=' + t.charset + '" method="post">';
...
QZFL.JSONGetter._runFnQueue = function(q, resultArgs, th){
	...
	//这里之前的时间点，移到回调的最前面，避免onSuccess执行的影响
...
QZFL.JSONGetter.prototype.send = function(callbackFnName) {
	...
	// 由于把request_xxx.js对接回qzfl了，所以这里需要做个兼容，不然会报错。
	this.url = da;
	...
	this._timer = setTimeout(
			(function(th){
				return function(){
						...
						th._waiting = false;
						th.onTimeout && th.onTimeout();
						QZFL.JSONGetter._pluginsRunner("onRequestTimeout", th);
					};
				})(this),
			this.timeout
		);
	...
	//if (this._charset == 'utf-8') {
		var win = QZFL.win,
			...
			extend = QZFL.object.extend;
		var isspeedup = cookieflag && (/(?:^user\.(s\d\.)?|\d{5,}\.|rc\.)qzone\.qq\.com/.test(host)),
			realurl = da + (function(oriurl){
				if(!QZFL.win.g_qzonetoken){
					return ''
				}
				if(oriurl.indexOf('?')>-1){ //有querystring
			...
			tmplURI,
			path,
			ajaxurl = isspeedup ? ('http://' + host + '/p') : realurl;
		// 测试是否可以走动态加速以及域名收归
        //关闭域名收归逻辑
        // gbk的输出没法转换，还是走jsonp吧。
		if (this._charset == 'utf-8' && ua.chrome  && window.QZFL && QZFL.JSONGetter && QZFL.config.xhrGetReqProxyEnable(da)) {
			tmplURI = QZFL.util.URI(realurl);
			var mDomain = tmplURI.host.replace('.qzone.qq.com', '');
            if(win.location.search){
                pIp = QZFL.getParameter(win.location.search, mDomain.split('.')[0]);
                if(pIp){
                    pIp = 'proxy_target_ip=' + pIp;
                    tmplURI.search ? (tmplURI.search += ('&' + pIp)) : (tmplURI.search += ('?' + pIp));
                }
            }

			// path = '/' + tmplURI.host.replace('.qzone.qq.com', '') + tmplURI.pathname + tmplURI.search;
			this.speedup = 1; //域名收归统计标记
			return (win.QZFL.ajaxGet(ajaxurl, null, {
				onload: (function(th){
					return function() {
						th._waiting = false;
						try{
	...
	if (QZFL.userAgent.ie && (typeof document.documentMode == 'undefined' || document.documentMode < 10) && !(QZFL.userAgent.beta && navigator.appVersion.indexOf("Trident\/4.0") > -1)) { // IE8之前的方案.确定要平稳迁移么
		...
		df['callback'] = df['_Callback'] = df[cfn] = (function(th){
				return (function(){
					th._waiting = false;
					th.endTime = +new Date;
					var cloneArguments = extend({}, arguments);
					var myArgs = cloneArguments;
					try{
						myArgs = _autoXSS(arguments);
					} catch(e) {
						myArgs = cloneArguments;
					}
					th.onSuccess.apply(th, myArgs);
					QZFL.JSONGetter._runFnQueue(th._squeue, myArgs, th);
					clear(th);
				});
			})(this);
...
QZFL.getParameter = function(url, name){
...
QZFL.pingSender = function(url, t, opts){
	...
	void((typeof(t) == 'number') ? setTimeout(function(){
		...
		img.src = transUrl2HttpsIfNeed(url);
	}, (t = Math.max(0, t))) : (img.src = transUrl2HttpsIfNeed(url)));
};
...
QZFL.media.reduceImage = (function(){
	var doReduce = function(o, type, ew, eh, p, cb){
		...
		o.parentNode && QZFL.lang.isFunction(cb) && cb(o, type, ew, eh, p);
	};
	...
	* @see QZFL.dialog.base
	...
	create : function(title, content, opts /*width,height,tween,noborder,top,left*/) {
		var t,
			args,
			dialog;
		if(content && typeof(content) == 'object' && content.src && content.src.indexOf('//')!==0 &&  content.src.indexOf('/')===0){
                            	content.src =window.location.protocol + '//'+ (window.imgcacheDomain || 'qzs.qq.com') + content.src;
		}else if(content && typeof(content)=='string'){
			content = content.replace(/<iframe([^>]+)>/ig,function(){
	                            var args = arguments;
	                            var attributeStr = args[1];
	                            return '<iframe '+attributeStr.replace(/src="([^"]+)"/i,function(){
	                            	var args = arguments;
	                                          var src = args[1];
	                                          if(src && src.indexOf('//')!==0 &&  src.indexOf('/')===0){
	                                          	src = window.location.protocol + '//'+ (window.imgcacheDomain || 'qzs.qq.com') + src;
				}
				return 'src="' + escHTML(src)+'"';
				})+'></iframe>';
	                            })
		}
...
//错误码上报统计
function errcodeReport(params) {
	params = params || {};
	params.cgi = params.cgi || '';
	params.code = params.code || 502;
	params.pathname = params.pathname || '';
	params.time = params.time || 0;

	var segments = params.cgi.split('/');
	var lastSegments = segments.pop();
	if(lastSegments && lastSegments.indexOf('?') > -1) {
		lastSegments = lastSegments.split('?')[0];
	}
	...
	if (XMLHttpRequest) {
		var xhr = new XMLHttpRequest();
	  	xhr.withCredentials = true;
	  	xhr.open('post', '//h5.qzone.qq.com/log/post/error/pc/502/' + lastSegments, true);
	  	xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
	  	xhr.send(data);
	}
}

function getCgiDomainAndPath(url) {
	var requestUrl = '';
	if(url.indexOf('?') > -1) {
	...
	requestUrl = requestUrl.replace(/\/\//, '').replace(/https?:/, '').replace(/(user|h5).qzone.qq.com\/proxy\/domain\//, '');
	var firstSlashIndex = requestUrl.indexOf('/');
	var upDomain = requestUrl.substring(0, firstSlashIndex);
	var upCgi = requestUrl.substring(firstSlashIndex) + '?qzfl';
	return {
		domain: upDomain,
		cgi: upCgi
	}
...
function returnCodeReport(opt) {};

//code 具体返回码
//type 0：成功，1:失败（这里指wns通道报错），2:逻辑失败
function reportMd (url, type, code, delay) {};
...
// url转https
//////////
function transUrl2HttpsIfNeed(ss, useSameProtocolOfPage){
    var sdms = QZFL.win.g_sdms || {};
    
        if(!ss){
            return ss;
        }
        
        var uri = QZFL.util.URI(ss);
        if(!uri){
            return ss;
        }
        ...
        var result;
        var proto;
	if(useSameProtocolOfPage){
	proto = window.location.protocol;
	}else{
        	proto = (QZFL.win.g_proto || window.location.protocol)
        }
        if(sdms[uri.host]==1){
            
            result = proto + '//'+(window.g_cgidomain || 'h5')+'.qzone.qq.com/proxy/domain/'+ uri.host +  uri.pathname+uri.search
                
        }else if(uri.host == 'qzonestyle.gtimg.cn' || uri.host == 'qzs.qq.com' || uri.host =='qzs.qzone.qq.com'){
        	result = proto + '//' + uri.host + uri.pathname + uri.search;
        }
     return result || ss;
}
...
(function(q){
	...
	var commurl = 'https://huatuocode.weiyun.com/code.cgi'
		...
		, rateNumG = (hours >0 && hours < 8 ) ? 1 : 1
		, rateNumP = (hours >0 && hours < 8 ) ? 1 : 1
		, isreportG = function(){
			return Math.random() * rateNumG < 1 // jsongetter成功时的抽样上报率	
		}
		, isreportP = function(){
			return Math.random() * rateNumP < 1 // formsender成功时的抽样上报率
		}
		...
		if (q[n] && q[n].pluginsPool) {
			...
			q[n].pluginsPool.onRequestComplete.push(function (th) {
				...
				if (th.msg && th.msg.indexOf('Connection') > -1) {
					collector.push([domain, url, 2, th.statusCode || 502, +th.endTime - th.startTime, 1, th.speedup]);
					return;
				}
				var d = th.resultArgs;
				if (d && (d = d[0])) {
					// 现在返回码改造不是重点了所以没有改造的cgi，把ret复制给code和subcode
					...
					if (d.code != 0) {
						collector.push([domain, url, 3, d.subcode || 1, +th.endTime - th.startTime, 1, th.speedup]);	
					}
					else {
						...
						if (th instanceof q.JSONGetter) {
							if (th.reportRate) {
								(th.reportRate == 1 || Math.random() < 1 / th.reportRate) && collector.push([domain, url, 1, d.subcode || 1, +th.endTime - th.startTime, th.reportRate || rateNumG, th.speedup]);	
							}
							else {
								isreportG() && collector.push([domain, url, 1, d.subcode || 1, +th.endTime - th.startTime, th.reportRate || rateNumG, th.speedup]);	
							}
						}
						// 成功按1/10抽样上报
						if (th && th.ownerWindow && th.ownerWindow.QZFL && th instanceof th.ownerWindow.QZFL.FormSender) {
							if (th.reportRate) {
			...
			// 加入测速统计
			q[n].pluginsPool.onRequestComplete.push(function (th) {
				if (isspeedup && th && th.ownerWindow && th.ownerWindow.QZFL && th instanceof th.ownerWindow.QZFL.FormSender) {
					...
					var stat_url = 'https://huatuospeed.weiyun.com/cgi-bin/r.cgi?flag1=175&flag2=363&flag3=' + fl3;
...
if(QZFL.userAgent.ie){ //一些浏览器行为矫正，IE9不支持重定义document!
	...
	try{
		if(!window.frameElement){ 
    			// 如果frameElement不存在，表明这个文件是在框架引入的。
    			// 当框架beforeunload的时候，需要去删除这个cookie，因为下次访问的框架节点不一定是收归节点
    ...
    QZFL.config.xhrProxyEnable = function(url) {
        if (location.href.indexOf("proxydebug") > -1) //强制开启写操作的域名收归
            return 1;
        if(location.href.indexOf("proxyoff")>-1){ //域名收归的开关
        	return false;
        }
        var mtches = url.match(urlParse), dm = mtches[1], cgi = mtches[2];
        var sdms = QZFL.win.g_sdms || {};
        if(sdms[dm]==1){//开启了https收归的，都不走域名收归逻辑
            return false;
        }
        
        return url.indexOf("taotao.qq.com") > -1
    };
    QZFL.config.xhrGetReqProxyEnable = function(url) {
        
        if (url.indexOf("//") == 0 || url.indexOf(".js") != -1)
            return false;
        if (url.charAt(0) == "/" || url.indexOf("../") == 0)
            return true;
        var sdms = QZFL.win.g_sdms || {};

        // var dms = [["u.qzone.qq.com", "*", 1], ["base.s2.qzone.qq.com", "*", 1], ["base.s8.qzone.qq.com", "*", 1], ["base.qzone.qq.com", "*", 1], ["r.qzone.qq.com", "*", 1], ["b.qzone.qq.com", "*", 1], ["b1.qzone.qq.com", "*", 1], ["m.qzone.qq.com", "*", 1], []].slice(0, 
        // -1);
        var mtches = url.match(urlParse), dm = mtches[1], cgi = mtches[2];
        if(sdms[dm]==1 || dm == location.hostname){//开启了https收归的，都不走域名收归逻辑
            return true;
        }
    ...
    QZFL.config.canUseXHR2 = function(url) {
        ...
        for (var i = 0; i < list.length; i++)
            if (list[i][0] == rst[1] && (new RegExp("\\/" + list[i][1] + "\\b")).test(url))
                return true;
        return false
    }
	QZFL.config.canUseXHR2Browser = function() {
        return !!(ua.chrome || ua.safari)
    }
